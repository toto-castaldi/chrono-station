...
022. **Falsa partenza.** Dopo lo Start, se l'operatore si accorge di un errore può **annullare** la partenza e tornare alla pagina di onboarding **mantenendo** squadre, membri ed esercizi (con i rispettivi ordini): non deve reinserire nulla. L'azione è disponibile durante l'esecuzione (countdown/running/paused) tramite un tasto **Annulla** con alert di conferma; azzera il progresso dell'allenamento (parziali registrati e pause per-squadra) e riporta lo stato a onboarding. È diversa dal reset «Nuovo allenamento» della pagina risultati (vedi doc/06 004), che invece svuota anche le squadre
